<template>
    <div class="app-container">
            ...
            <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left"
              ...
              <el-form-item label="地址" prop="address">
                ...
                <el-form-item label="内景图">
                    <ul class="ve">
                        <li v-for="(item,index) in dataForm.pictures" v-if="!item.video">
        ...
        <!-- 商品添加对话框 -->
        <el-dialog :title="'添加商品'" :visible.sync="goodDialogVisible" :close-on-click-modal="false">
            <el-form ref="goodDataForm" :rules="rules" :model="goodDataForm" status-icon label-position="left"
                ...
                <el-form-item label="宣传图片">
                 ...
                 <el-form-item label="商品简介" prop="brief">
                    <el-input type="textarea" :rows="6" v-model="goodDataForm.brief"/>
                </el-form-item>
            </el-form>
...
<script>
    ...
    import {uploadBrandIcon, uploadBrandIndoorPic, uploadGoodsIcon, uploadGoodsDetailPic ,uploadBrandVideo} from '@/api/fileUpload'
...
//确定上传
            submitUpload5() {
                ...
                var self = this
                uploadBrandVideo(this.fileData5).then(function (response) {
                    console.log(11111)
                    console.log(response)
                ...
                uploadGoodsDetailPic(this.fileData3).then(function (response) {
                    ...
                    for (var i in self.goodDataForm.pictures) {
                        if (self.goodDataForm.pictures[i].picUrl.substr(0, 4) != 'http') {
                            self.goodDataForm.pictures[i].picUrl =  self.goodDataForm.pictures[i].picUrl
                        }
                    }
...
//确定上传
            submitUpload4() {
                ...
                uploadGoodsIcon(this.fileData4).then(function (response) {
                    ...
                    self.goodDataForm.picUrl =  response.data.data[0]
...
//获取商家类目列表
            ...
            handleUpdate(row) {
                ...
                this.dialogFormVisible = true
                console.log(row.pictures[0])
                if(row.pictures[0].video==1){
