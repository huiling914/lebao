    ...
    <div class="app-container" >
        ...
        <div style="height: calc(100% - 155px);overflow: hidden">
            <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit
                ...
                <el-table-column align="center" :formatter="formatterColumn" label="类目类型" prop="type"/>
                ...
                <el-table-column
                    ...
                    prop="level">
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.level === 'L1' ? 'primary' : 'info' ">{{ scope.row.level === 'L1' ? '一级类目'
                            : '二级类目' }}
                        </el-tag>
                    </template>
                </el-table-column>
...
//确定上传
            submitUpload() {
                ...
                uploadCategoryIcon(this.fileData).then(function (response) {
                    ...
                    self.dataForm.iconUrl =  response.data.data[0]
                ...
                .then(response => {
                    ...
                    for (var i in this.list) {
                        if (this.list[i].iconUrl.substr(0, 4) != 'http') {
                            this.list[i].iconUrl =  this.list[i].iconUrl
                        }
                    }
            ...
            handleSelectionDelete: function () {
                ...
                if(this.deletelist.length==0){
                    ...
                    deleteCategory(this.deletelist).then(response => {
                    this.$notify.success({
                        title: '成功',
                        message: '删除成功'
                    })
